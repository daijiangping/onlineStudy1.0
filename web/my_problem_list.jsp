...
<link rel="apple-touch-icon-precomposed" href="msite/images/5da37614.apple-touch-icon-precomposed.png">
    <style>
        .deletes{
            float: right;width: 20px;margin-top: 30px;
        }
    </style>
</head>
<body>
...
<div class="main">
    <!--管理员删除-->
    <input type="hidden" class="delete-flag" value="${op}">
    <ul class="post-list">
        ...
        <li>
            <img src="icon/delete-icon.png" data-id="${p.id}" class="deletes" onclick="deleteProblem(this);"/>
                <a style="width: 90%" href="${pageContext.request.contextPath}/problem!findBySortIdAndId.action?problem.id=${p.id}" class="post">
                <h4>${p.title}</h4>
                <span class="post-info">
                    <span>
                        <c:if test="${op=='adminDelete'}">
                          <span  style="float: left">
                              <c:out value="${p.user.realName}"></c:out></span>
                    </c:if>&nbsp;&nbsp;&nbsp;
                    ${p.answerNumber} 回应</span>
                    <i class="split">|</i>
    ...
    <c:if test="${not empty pageMap.dataList}">
        <ul class="pager">
            <c:choose>
                ...
                <c:otherwise>
                    <li><a href="${pageContext.request.contextPath}/problem!findMyProblem.action?pageNo=1">首页</a></li>
                    <li><span><a href="${pageContext.request.contextPath}/problem!findMyProblem.action?pageNo=${pageMap.pageNo-1}"> 上一页 </a></span></li>
                </c:otherwise>
            </c:choose>
            <c:choose>
                ...
                <c:otherwise>
                    <li><a href="${pageContext.request.contextPath}/problem!findMyProblem.action?pageNo=${pageMap.pageNo+1}">下一页</a></li>
                    <li><a href="${pageContext.request.contextPath}/problem!findMyProblem.action?pageNo=${pageMap.totalPage}">末页</a></li>
                </c:otherwise>
            </c:choose>
        </ul>
    </c:if>
...
<script>
function deleteProblem(the) {
   var r =  confirm("你确定要删除吗？");
    if(r){
        var id  = $(the).attr("data-id");
        $(the).parent("li").remove();
        var flag = $(".delete-flag").val();
        var url;
       if(flag=="adminDelete"){
           url = "${pageContext.request.contextPath}/problem!deleteMyProblem.action?problem.id="+id+"&op=adminDelete";
       }else{
           url = "${pageContext.request.contextPath}/problem!deleteMyProblem.action?problem.id="+id;
       }
        $.post(url);
        alert("已删除！");
    }else{
